...
class Keyboards:
    ...
    def search_menu() -> str:
        keyboard = VkKeyboard()
        keyboard.add_button('Расписание на определенный день',
    ...
    def settings_menu(user: User) -> str:
        ...
        if (user.subscription_days is None or user.subscription_days == const.CHANGES) and user.current_name:
            keyboard.add_button('Подписаться на расписание', payload={const.PAYLOAD_MENU: const.MENU_SUBSCRIBE})
            keyboard.add_line()
        else:
            if user.subscription_days is not None and user.subscription_days != const.CHANGES and user.current_name:
                if user.current_name:
                    keyboard.add_button('Изменить подписку на расписание',
    ...
    def subscribe_to_schedule_day_menu(user: User) -> str:
        ...
        keyboard.add_button('Текущий день',
        ...
        keyboard.add_line()
        keyboard.add_button('Текущий и следующий день',
                            payload={const.PAYLOAD_MENU: const.MENU_UPDATE_SUBSCRIPTION,
